...
Code blocks modified for the thesis are marked with the comment `Thesis`. 
The rest is ParlAI code quoted for context.
"""
...
import sys
from parlai.scripts import config as my_config # Thesis 
...
class TorchGeneratorAgent(TorchAgent, ABC):
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        ##########
        self.step_count = 0 
        self.dest = my_config.save_path 
        self.task_id  = my_config.identifier 
        self.compare_mode = my_config.compare_mode 
        self.compare_with = my_config.compare_with 
        self.decay_const = my_config.decay_const 
        self.skip_save = my_config.skip_save 
        self.fixed_scale_mode = my_config.fixed_scale_mode
        self.floored_scale_mode = my_config.floored_scale_mode
        self.fixed_scale_const = my_config.fixed_scale_const
        
        print('init compare mode: ', my_config.compare_mode)
        print('init decay constant: ', my_config.decay_const)
        print('init fixed scale mode: ', my_config.fixed_scale_mode)
        print('init floored scale mode: ', my_config.floored_scale_mode)
        print('init fixed scale constant: ', my_config.fixed_scale_const)
    ...
    def compute_loss(self, batch, return_output=False):
        ...
        scores, preds, *_ = model_output # scores [1, 6, 54944], pred [1, 6]
        score_view = scores.reshape(-1, scores.size(-1)) # [6, 54944]
    ...
    def eval_step(self, batch):
        ...
        if self.skip_generation:
            ...
            beam_preds_scores, beams = self._generate(batch, self.beam_size, maxlen) #len(beam_preds_scores[0])=2, first tensor torch.Size([25]), second is tensor(-7.7385))

            preds, scores = zip(*beam_preds_scores) # word indices preds[0].size()=torch.Size([25]), scores=(tensor(-7.7385),)
            self._add_generation_metrics(batch, preds)

            ##########
            # Thesis
            ##########
            # compute perplexity without target sequence
            my_ppl = [sum(PPLMetric.many([-score[0]*score[0].exp() for score in beam.all_scores]), None) for beam in beams]
            print('my ppl: ', my_ppl[0].value())

            # bsz x beamsize
            beam_texts: List[List[Tuple[str, float]]] = []
            for beam in beams:
                beam_texts.append([])
                for tokens, score in beam.get_rescored_finished(): # len beam_size
                    try:
    ...
    def _score_tilde(self, score, other):
        ##########
        # Thesis
        ##########
        assert score.size()==other.size()
        ...
        if self.fixed_scale_mode:
            # fixed scaling factor (simpler)
            factor = torch.mul(ones,self.fixed_scale_const)
        elif self.floored_scale_mode:
            # elementwise max of fixed scale and exponential
            floor = torch.mul(ones,self.fixed_scale_const)
            exponential = torch.exp(torch.mul(delta,self.decay_const))
            factor = torch.maximum(floor, exponential)
        else:
            # exponential scaling factor (paper)
            factor = torch.exp(torch.mul(delta,self.decay_const))

        scaling = torch.where(delta>=0, ones, factor)
        # elementwise multiplication
        score_tilde = torch.mul(scaling, score)
    ...
    def _generate(
        ...
        for _ts in range(max_ts):
            ...
            # Thesis
            ##########
            # scale probabilities before taking log
            score = F.softmax(score, dim=-1, dtype=torch.float32)
            # print('score: ', score.size()) # torch.Size([1, 3, 54944]) current word's log probability distribution
            if not self.skip_save and not self.compare_mode:
                # save original probabilities for comparison next time
                tensor_save_path = self.dest+self.task_id+'/'+str(self.step_count)+'_'+str(_ts)+'.pt'
                torch.save(score, tensor_save_path)
            if self.compare_mode:
                # load the corresponding tensor 
                # print('example {}, word {}'.format(self.step_count,_ts))
                other_path = self.dest+self.compare_with+'/'+str(self.step_count)+'_'+str(_ts)+'.pt'
                try:
                    other = torch.load(other_path)
                    score = self._score_tilde(score, other) # for eval_model score is [1, 1, 54944] instead of [1, 3, 54944] if not set beam size 3
                except FileNotFoundError as e:
                    pass # to prevent excess log
                except:
                    print("Unexpected error:", sys.exc_info()[0])
                    raise
            # log likelihood
            score = torch.log(score)
            # comment out F.log_softmax
        ...
        n_best_beam_preds_scores = [b.get_rescored_finished() for b in beams] # len beam_size, each is a tuple (torch.Size([25]), tensor(-7.7385))
