...
import sys # Thesis
import logging as loggingPackage # Thesis
from parlai.scripts import helper # Thesis
...
class TorchGeneratorModel(nn.Module, ABC):
    ...
    def __init__(
        ...
        # Thesis
        ##########
        self.logger = self.my_logger(helper.OUTFILE)
        print('random_num: ', helper.random_num)
        print('outfile: ', helper.OUTFILE)

    def my_logger(self, OUTFILE):
        """
        Thesis
        Logger for saving log-likelihood to file
        """
        logger = loggingPackage.getLogger()
        logger.setLevel(loggingPackage.DEBUG)

        output_file_handler = loggingPackage.FileHandler(OUTFILE)
        stdout_handler = loggingPackage.StreamHandler(sys.stdout)
    ...
    def decode_forced(self, encoder_states, ys):
        ...
        latent, _ = self.decoder(inputs, encoder_states) # torch.Size([1, 5, 512])
        logits = self.output(latent) # torch.Size([1, 5, 54944]) when display_model, batch size 1
        _, preds = logits.max(dim=2) # preds are tokens in vocab with highest logit score
        
        ##########
        # Thesis
        ##########        
        s = nn.Softmax(dim=1) # apply softmax so all vocab's logits sum to 1
        logits_view = torch.reshape(logits, (logits.size(-2), logits.size(-1)))
        probabilities = s(logits_view) 
        target_indices = ys[0,:]
        ys_probabilities = probabilities.gather(1, target_indices.view(-1,1))
        target_likelihood = torch.prod(ys_probabilities)
        # print('target_likelihood: ', target_likelihood.item())
        self.logger.debug(target_likelihood.item()) # log 
